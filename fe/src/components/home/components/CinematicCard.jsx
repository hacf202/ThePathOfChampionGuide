...
const CinematicCard = ({ to, icon: Icon, title, desc, img, small = false }) => {
	return (
		<div variants={fadeInUp} className="h-full group cursor-pointer" transition={{ duration: 0.4, ease: "easeOut" }}>
			...
			>
				<img src={img} alt={title} loading='lazy' className="absolute inset-0 w-full h-full object-cover opacity-100 transition-all duration-[1s] ease-out group-hover:scale-110 pointer-events-none" />
				{/* Đảm bảo gradient đen đủ đậm để đọc chữ dễ dàng */}
				<div className="absolute inset-0 bg-gradient-to-t from-[#05050A] via-[#05050A]/70 to-transparent pointer-events-none opacity-50 group-hover:opacity-70 transition-opacity duration-500" />
