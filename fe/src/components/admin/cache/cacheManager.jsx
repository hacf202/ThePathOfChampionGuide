...
const CacheManager = () => {
    ...
    const handleClearAll = async () => {
        ...
        try {
            ...
            setMessage({ type: "success", text: tUI("admin.cache.clearSuccess") });
            fetchStats();
            setShowConfirm(false);
        } catch (error) {
            setMessage({ type: "error", text: error.response?.data?.error || tUI("admin.common.errorOccurred") });
        } finally {
    ...
    const handleClearCache = async (name) => {
        ...
        try {
            ...
            setMessage({ type: "success", text: tUI("admin.cache.purgeSuccess", { name }) });
            fetchStats();
        } catch (error) {
            setMessage({ type: "error", text: error.response?.data?.error || tUI("admin.cache.purgeError", { name }) });
        } finally {
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div className="p-6 bg-surface-bg border border-border rounded-2xl shadow-sm hover:shadow-md transition-shadow">
                    <div className="flex items-center gap-4 mb-4">
                        ...
                        <div>
                            <p className="text-xs text-text-tertiary uppercase tracking-wider font-bold">{tUI("admin.cache.totalModules")}</p>
                ...
                <div className="p-6 bg-surface-bg border border-border rounded-2xl shadow-sm hover:shadow-md transition-shadow">
                    <div className="flex items-center gap-4 mb-4">
                        ...
                        <div>
                            <p className="text-xs text-text-tertiary uppercase tracking-wider font-bold">{tUI("admin.cache.totalRecords")}</p>
                     ...
                     {!showConfirm ? (
                        ...
                        <div className="flex flex-col gap-3 w-full">
                            <p className="text-sm font-bold text-red-400 text-center flex items-center justify-center gap-2">
                                <ShieldAlert size={16} /> {tUI("admin.cache.confirm")}
                            </p>
                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="bg-surface-hover/50 text-text-tertiary text-xs uppercase tracking-widest font-bold">
                            <th className="px-6 py-4">{tUI("admin.cache.moduleName")}</th>
                            <th className="px-6 py-4 text-center">{tUI("admin.cache.cachedKeys")}</th>
                            <th className="px-6 py-4 text-center">{tUI("admin.cache.hits")}</th>
                            <th className="px-6 py-4 text-center">{tUI("admin.cache.misses")}</th>
                            <th className="px-6 py-4 text-right">{tUI("admin.cache.actions")}</th>
                        </tr>
                    </thead>
                        ...
                        {stats.length > 0 ? stats.map((item) => (
                            <tr key={item.name} className="hover:bg-surface-hover/30 transition-colors">
                                ...
                                <td className="px-6 py-5 text-right">
                                    <button
                                        ...
                                        title={tUI("admin.cache.purgeTitle", { name: item.name })}
                            ...
                            <tr>
                                <td colSpan="5" className="px-6 py-12 text-center text-text-tertiary italic">
                                    {loading ? tUI("admin.cache.statsLoading") : tUI("admin.cache.statsEmpty")}
                                </td>
                            </tr>
                        )}
                    </tbody>
