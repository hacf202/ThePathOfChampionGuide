import { useState, useEffect, memo, useMemo } from "react";
import Button from "../common/button";
import InputField from "../common/inputField";
import GenericCard from "../common/genericCard";
import { useCrudEditor } from "../../hooks/useCrudEditor";
import EditorLayout from "../common/editorLayout";

const RelicEditorForm = ({ relic, onSave, onCancel, onDelete, isSaving }) => {
	const [formData, setFormData] = useState(relic);
	useEffect(() => setFormData(relic), [relic]);
	const handleChange = e =>
		setFormData(prev => ({ ...prev, [e.target.name]: e.target.value }));

	return (
		<div className='p-4 bg-surface-bg'>
			<div className='flex justify-between mb-6 pb-4 border-b border-border'>
				<h3 className='text-xl font-bold'>
					{relic.isNew ? "Tạo Cổ Vật Mới" : relic.name}
				</h3>
				<div className='flex gap-2'>
					{!relic.isNew && (
						<Button variant='danger' onClick={onDelete}>
							Xóa
						</Button>
					)}
					<Button variant='outline' onClick={onCancel}>
						Hủy
					</Button>
					<Button
						variant='primary'
						onClick={() => onSave(formData)}
						disabled={isSaving}
					>
						{isSaving ? "Lưu..." : "Lưu"}
					</Button>
				</div>
			</div>
			<div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
				<div className='flex flex-col gap-4'>
					<InputField
						label='Mã:'
						name='relicCode'
						value={formData.relicCode}
						onChange={handleChange}
						disabled={!relic.isNew}
					/>
					<InputField
						label='Tên:'
						name='name'
						value={formData.name}
						onChange={handleChange}
					/>
					<InputField
						label='Độ hiếm:'
						name='rarity'
						value={formData.rarity}
						onChange={handleChange}
					/>
					<InputField
						label='Độ hiếm tham chiếu:'
						name='rarityRef'
						value={formData.rarityRef}
						onChange={handleChange}
					/>
					<InputField
						label='Stack:'
						type='number'
						name='stack'
						value={formData.stack}
						onChange={handleChange}
					/>
					<InputField
						label='Loại (Type):'
						name='type'
						value={formData.type}
						onChange={handleChange}
					/>
				</div>
				<div className='flex flex-col gap-4'>
					<InputField
						label='Đường dẫn Ảnh:'
						name='assetAbsolutePath'
						value={formData.assetAbsolutePath}
						onChange={handleChange}
					/>
					<InputField
						label='Đường dẫn Ảnh đầy đủ:'
						name='assetFullAbsolutePath'
						value={formData.assetFullAbsolutePath}
						onChange={handleChange}
					/>
					<div className='flex flex-col gap-1'>
						<label className='text-sm font-medium text-text-secondary'>
							Mô tả:
						</label>
						<textarea
							name='description'
							rows={3}
							value={formData.description}
							onChange={handleChange}
							className='p-2 bg-input-bg rounded border border-input-border text-input-text'
						/>
					</div>
					<div className='flex flex-col gap-1'>
						<label className='text-sm font-medium text-text-secondary'>
							Mô tả Thô:
						</label>
						<textarea
							name='descriptionRaw'
							rows={3}
							value={formData.descriptionRaw}
							onChange={handleChange}
							className='p-2 bg-input-bg rounded border border-input-border text-input-text'
						/>
					</div>
				</div>
			</div>
		</div>
	);
};

function RelicEditor() {
	// Custom Filter Logic cho Relic (Type và Stack)
	const customFilterLogic = (data, filters) => {
		let res = data;
		if (filters.types && filters.types.length > 0)
			res = res.filter(r => filters.types.includes(r.type));
		if (filters.stacks && filters.stacks.length > 0)
			res = res.filter(r => filters.stacks.includes(r.stack));
		return res;
	};

	const { state, actions } = useCrudEditor({
		endpoint: "relics",
		idField: "relicCode",
		routePath: "/admin/relics",
		newItemTemplate: {
			isNew: true,
			name: "Cổ Vật Mới",
			rarity: "",
			description: "",
			type: "Relic",
			stack: 1,
		},
		customFilterLogic,
	});

	const filterConfigs = useMemo(
		() => ({
			multiFilters: [
				{
					label: "Độ hiếm",
					placeholder: "Tất cả",
					options: [...new Set(state.data.map(i => i.rarity))]
						.sort()
						.map(r => ({ value: r, label: r })),
					selectedValues: state.selectedRarities,
					onChange: actions.setSelectedRarities,
				},
				{
					label: "Loại",
					placeholder: "Tất cả",
					options: [...new Set(state.data.map(r => r.type || "Other"))]
						.sort()
						.map(t => ({ value: t, label: t })),
					selectedValues: state.customFilterValues.types || [],
					onChange: vals =>
						actions.setCustomFilterValues(p => ({ ...p, types: vals })),
				},
				{
					label: "Stack",
					placeholder: "Tất cả",
					options: [...new Set(state.data.map(r => r.stack))]
						.sort((a, b) => a - b)
						.map(s => ({ value: s, label: `Stack ${s}` })),
					selectedValues: state.customFilterValues.stacks || [],
					onChange: vals =>
						actions.setCustomFilterValues(p => ({ ...p, stacks: vals })),
				},
			],
		}),
		[state.data, state.selectedRarities, state.customFilterValues, actions]
	);

	return (
		<EditorLayout
			state={{ ...state, idField: "relicCode" }}
			actions={actions}
			filterConfigs={filterConfigs}
			placeholders={{ search: "Tìm cổ vật...", add: "Thêm Cổ Vật" }}
			renderCard={item => (
				<GenericCard
					key={item.relicCode}
					item={item}
					onClick={() => actions.handleSelect(item.relicCode)}
				/>
			)}
			renderForm={item => (
				<RelicEditorForm
					relic={item}
					onSave={actions.handleSave}
					isSaving={state.isSaving}
					onCancel={() => actions.handleAttemptClose(false)}
					onDelete={() => {
						actions.setItemToDelete(item);
						actions.setModals(p => ({ ...p, delete: true }));
					}}
				/>
			)}
		/>
	);
}
export default memo(RelicEditor);
