...
function ItemList() {
	...
	const {
		...
		pagination,
		dynamicFilters,
	} = useGenericData("items", queryParams, tUI, "itemCode");
	...
	useEffect(() => {
		if (knownItems && knownItems !== tempKnownItems) {
			setTempKnownItems(knownItems);
		}
	}, [knownItems, tempKnownItems]);
	...
	return (
		<GenericListLayout
			...
			renderItem={item => {
				...
				const itemRarityTranslated = getTranslatedRarity(item.rarity, item);
					...
					>
						...
						<div className='flex-grow overflow-hidden'>
							...
							<div className='flex items-center gap-2 text-sm text-text-secondary'>
								...
								<span className='truncate'>{itemRarityTranslated}</span>
							</div>
						</div>
