...
function RuneList() {
	...
	const [knownRunes, setKnownRunes] = useState([]);
	...
	// --- PERSISTENT STATE (CHỈ GIỮ LẠI SEARCH VÀ RARITY) ---
	...
	const fetchRunes = useCallback(async () => {
		...
		try {
			...
			setRunes(data.items || []);
			setKnownRunes(prev => {
				const map = new Map(prev.map(r => [r.runeCode, r]));
				(data.items || []).forEach(r => map.set(r.runeCode, r));
	...
	const getTranslatedRarity = useCallback(
		(rawRarity, item) => {
			...
			return tUI(`rune.rarity.${rawRarity.toLowerCase()}`);
		},
		[tUI, t],
	);
	...
	const filterOptions = useMemo(
		() => ({
			rarities: (dynamicFilters.rarities || []).map(r => {
				const sample = knownRunes.find(run => run.rarity === r);
				return {
					value: r,
															...
															<div className='flex-grow overflow-hidden'>
																...
																<div className='flex items-center gap-2 text-sm text-text-secondary'>
																	...
																	<span className='truncate'>
																		{getTranslatedRarity(rune.rarity, rune)}
																	</span>
																</div>
															</div>
