...
function RelicDetail() {
	...
	const compatibleChampions = useMemo(() => {
		...
		return champions
			.filter(champ =>
				champ.relicSets?.some(set => {
					if (Array.isArray(set)) return set.includes(relic.relicCode);
					if (set && Array.isArray(set.items)) return set.items.includes(relic.relicCode);
					return false;
				})
			)
			.map(champ => ({
