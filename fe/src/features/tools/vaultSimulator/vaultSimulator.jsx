...
import { ASSETS, VAULT_CONFIG } from "@/features/tools/vaultSimulator/components/vaultConfig";
import VaultProbabilityInfo from "@/features/tools/vaultSimulator/components/VaultProbabilityInfo";
...
const VaultOpeningAnimation = ({ src }) => {
	...
	return (
		<div ref={containerRef} className='relative w-64 h-64 z-50'>
			<img
				src={src}
				alt='Vault Opening'
				className='w-full h-full object-contain filter drop-shadow-[0_0_80px_rgba(var(--primary-500-rgb),0.5)]'
			/>
			<div className='absolute inset-0 bg-primary-500/30 blur-[100px] animate-pulse rounded-full pointer-events-none' />
		</div>
	);
};
	...
	const LootItem = ({ item, isSmall = false }) => {
		const getDisplayName = () => {
			if (item.refType === "champion") {
				...
				if (champ) return `${tUI("vaultSimulator.loot.champ_frags")} ${t(champ, "name") || champ.name}`;
			...
			<div className='max-w-7xl mx-auto py-4 sm:py-6 px-2 sm:px-4 lg:px-8 relative z-10'>
				...
				{/* Selection Modal */}
					...
					{showConfigModal && (
						<div 
							...
							className='fixed inset-0 z-[100] flex items-center justify-center p-2 sm:p-4 bg-black/30 overflow-y-auto-scrollbar'
						>
							<div
				...
				{isOpening && (
					<div className='flex flex-col items-center justify-center py-24'>
						<VaultOpeningAnimation src={ASSETS[selectedVault]} />
						<h2
					...
					{loot && !isOpening && (
						...
						>
							...
							<StaggerContainer className='grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-8 sm:gap-16 relative z-10 justify-items-center'>
									{loot.map((item, idx) => (
										<StaggerItem
											key={`${item.id}-${idx}`}
											className='w-full'
				...
				{/* Probability Modal */}
					...
					{showProbModal && (
						<div 
							...
							className='fixed inset-0 z-[200] flex items-center justify-center p-2 sm:p-4 bg-black/60 backdrop-blur-sm overflow-y-auto-scrollbar select-text'
						>
							<div
								onClick={(e) => e.stopPropagation()}
							...
							>
								<div className='overflow-y-auto max-h-[80vh]-scrollbar'>
