...
### ✅ Hoàn tất Tích hợp Hệ thống Tài nguyên & Rương

- **Tích hợp Trang chủ (Home Page)**:
  - Thêm lối tắt "Tài nguyên" vào **Hero Section** (ô gạch bay), **Database Grid** (thẻ bài) và **Quick Tools** (thanh công cụ nhanh).
  - Đồng bộ hóa đa ngôn ngữ (`lootTitle`, `lootDesc`) cho cả VI và EN.
- **Tối ưu hóa Giao diện Di động (Mobile UI)**:
  - **DataTable**: Thu nhỏ cỡ chữ (10px/11px) và giảm padding trên mobile nhằm tối đa hóa diện tích hiển thị.
  - **Resource Tables**: 
    - Thu gọn các cột cố định ("Bậc", "Loại", "Hành động") từ 200px xuống 120-140px.
    - Ép `whitespace-nowrap` cho các cụm tài nguyên (số lượng + icon) để ngăn ngắt dòng vụn vặt và kích hoạt cuộn ngang (`min-width`).
- **Nâng cấp UX/UI**:
  - **Điều hướng**: Thay đổi Icon đại diện của Tài nguyên từ `Sparkles` sang `Book` (Lucide) để mang tính "tra cứu" chuyên nghiệp hơn.
  - **Bộ lọc**: Thêm tooltip hướng dẫn phím tắt "Ấn TAB để ẩn/hiện bộ lọc" cho nút Filter.
- **Sửa lỗi & Ổn định hệ thống**:
  - **Build Error**: Khắc phục lỗi import sai tệp `markupStripper` (thực tế là `markupUtils`) trong `MarkupEditor.jsx`.
  - **Runtime Crash**: Thêm giá trị mặc định cho `renderSkeleton` trong `GenericListLayout.jsx` để ngăn lỗi crash khi trang Tài nguyên đang tải dữ liệu.
- **Dữ liệu (Data)**: Chuẩn hóa toàn bộ nội dung trong `resources.json` sang định dạng Markup (`[res:id|text]`) giúp kích hoạt tooltip liên kết chéo tự động.
...
## Log thay đổi 2026-04-12 (Vault Simulator UI/UX Finalization) _(Uncommitted)_
...
- **Thuật Ngữ**: Đổi danh pháp "Mảnh Ghép Bí Ẩn" thành "Mảnh ghép bí ẩn". Tháo gỡ các block dịch tỉ lệ gộp rương ở bộ ngôn ngữ và định nghĩa chi tiết chuyên biệt hóa 1-1 cho từng rương.
...
_Không có bug hay blocking nghiêm trọng nào._
